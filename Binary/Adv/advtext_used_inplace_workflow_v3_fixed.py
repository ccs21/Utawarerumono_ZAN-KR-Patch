...
SPECIAL_MARKER = "#@－－#"     # must be preserved verbatim
SPECIAL_TOKEN = "{{LOCK_AT_DASH}}"  # placeholder shown in CSV to reduce accidental edits
SPECIAL_LOCKS = [SPECIAL_MARKER, SPECIAL_TOKEN]

# Ruby/furigana tag: <KANJI:READING> (we can optionally strip it to make translation easier)
RE_RUBY = re.compile(r"<([^<>:]+):([^<>]+)>")

def strip_ruby_tags(text: str) -> str:
    """Convert ruby tags like <帝:ミカド> into just the base text '帝'."""
    return RE_RUBY.sub(lambda m: m.group(1), text)

def protect_special_marker(text: str) -> str:
    return text.replace(SPECIAL_MARKER, SPECIAL_TOKEN)

# Accept common user typos for the marker, normalize to SPECIAL_MARKER.
RE_MARKER_VARIANTS = re.compile(r"#@\s*[-－ー‐‑–—]{2,}\s*#")

def normalize_marker_variants(text: str) -> str:
    return RE_MARKER_VARIANTS.sub(SPECIAL_MARKER, text)

def tokenise_marker(text: str) -> str:
    # normalize variant forms then tokenise
    return protect_special_marker(normalize_marker_variants(text))

def detokenise_marker(text: str) -> str:
    # accept both token and marker and output marker
    text = normalize_marker_variants(text)
    return text.replace(SPECIAL_TOKEN, SPECIAL_MARKER)
...
def collect_used_slots(advtext_path: Path, script_paths: List[Path], step: int = 1,
                       ...
                       min_budget_bytes: int = 8, min_ref_count: int = 1,
                       ruby: str = 'strip', protect_special: bool = True) -> Dict[int, Slot]:
    ...
    for start, ref_count in counts.items():
        ...
        if s is None:
            continue
        # Optional: strip ruby/furigana tags to avoid forcing translators to keep them
        if ruby == 'strip':
            s = strip_ruby_tags(s)
        # Optional: show special marker as a safe placeholder in CSV
        if protect_special:
            s = tokenise_marker(s)
...
def strict_lock_check(src_ja: str, dst_text: str) -> List[str]:
    ...
    # Compare in token space (so {{LOCK_AT_DASH}} is handled even if user typed variants)
    dst_norm = tokenise_marker(dst_text)
    missing = [lk for lk in required if lk not in dst_norm]
    return missing
...
def apply_inplace_patch(
    ...
    for idx, row in enumerate(rows, start=2):  # +1 for header; start=2 matches Excel-ish row number
        ...
        if kr is None:
            kr = ""
        kr = kr.replace("\r\n", "\n").replace("\r", "\n")
        # Normalize and tokenise special marker (#@－－#)
        kr = tokenise_marker(kr)
        ...
        if fail_on_lock_missing:
            ...
            if missing_locks:
                raise PatchError(
                    ...
                    f"JA={ja}\n"
                    f"KR={kr}\n"
                    f"MAPPED={mapped_final}"
                )

        # Encode and size check (detokenise marker back to real bytes)
        mapped_final = detokenise_marker(mapped)
        new_bytes = mapped_final.encode("utf-8")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_ex.add_argument("--ruby", choices=["keep","strip"], default="strip",
              help="How to handle ruby/furigana tags like <帝:ミカド>. 'strip' keeps only base text (default).")
    p_ex.add_argument("--no-protect-special", action="store_true",
              help="Do not replace special marker #@－－# with a CSV-safe token (default: protected as {{LOCK_AT_DASH}}).")
