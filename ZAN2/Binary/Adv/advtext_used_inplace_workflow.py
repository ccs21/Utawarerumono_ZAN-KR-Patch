...
def looks_like_text(s: str) -> bool:
    """Heuristic: exclude obvious binary garbage. Keep CJK / punctuation heavy strings."""
    if not s:
        return False
    # Must contain at least one non-ASCII printable or typical CJK punctuation.
    if any(ord(c) >= 0x80 for c in s):
        return True
    # Or contains common punctuation with letters/numbers
...
def collect_used_slots(advtext_path: Path, script_paths: List[Path], step: int = 1,
                       require_text: bool = True) -> Dict[int, Slot]:
    ...
    for start, ref_count in counts.items():
        ...
        if s is None:
            continue
        if require_text and not looks_like_text(s):
            continue
