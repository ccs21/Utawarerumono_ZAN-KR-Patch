...
def apply_translations(
    ...
    *,
    ...
    off_to_group: Dict[int, Tuple[str, str]] = {}  # offset -> (group_key, jp_type)
    for r in jp_rows:
        gk = r.get("group_key", "")
        if not gk:
            continue
        jp_type = r.get("jp_type", "")
        off = int(r["offset"])
        off_to_group[off] = (gk, jp_type)
        if only_normal and jp_type != "normal":
            continue
    ...
    for row_no, r in enumerate(diag_rows, start=2):  # CSV line number (header=1)
        gk = r.get("group_key", "")
        if only_normal and r.get('jp_type','') != 'normal':
            continue
        if not gk:
            continue
        ...
        for off in targets:
            ...
            if not ok:
                if auto_fix:
                    ...
                    if not ok2:
                        gk2, jt2 = off_to_group.get(off, (gk, ''))
                        meta2 = trans_meta.get(gk2, {})
                        info = {
                            'offset_hex': f"0x{off:X}",
                            'offset_dec': str(off),
                            'group_key': gk2,
                            'jp_type': jt2,
                            'row_no': meta2.get('row_no',''),
                            'rep_id_hex': meta2.get('rep_id_hex',''),
                            'missing_tokens': '|'.join(missing2),
                            'src_preview': format_preview(src),
                            'dst_preview': format_preview(dst),
                        }
                        try:
                            write_lock_fail_report_csv('talkspt_lock_fail_report.csv', info)
                        except Exception:
                            pass
                        raise ValueError(f"[LOCK FAIL] offset=0x{off:X} group_key={gk2} row={info['row_no']} missing={missing2} :: src={info['src_preview']} dst={info['dst_preview']}")
                    dst = dst2
                else:
                    gk2, jt2 = off_to_group.get(off, (gk, ''))
                    meta2 = trans_meta.get(gk2, {})
                    ...
                    try:
                        ...
                        pass
                    raise ValueError(f"[LOCK FAIL] offset=0x{off:X} group_key={gk2} row={info['row_no']} missing={missing} :: src={info['src_preview']} dst={info['dst_preview']}")
...
def main():
    ...
    a.add_argument("--only-normal", action="store_true",
                   help="jp_type=normal(가나 포함)만 적용/패치합니다. 중국어/한자-only/영어는 제외하고 싶을 때 사용.")
...
def write_lock_fail_report_csv(path: str, item: dict) -> None:
    """Write a single lock-fail diagnostic row as UTF-8-SIG CSV."""
    import csv
    headers = [
        "offset_hex","offset_dec","group_key","jp_type","row_no","rep_id_hex",
        "missing_tokens","src_preview","dst_preview"
    ]
    with open(path, "w", encoding="utf-8-sig", newline="") as f:
        w = csv.writer(f, quoting=csv.QUOTE_ALL)
        w.writerow(headers)
        w.writerow([item.get(h, "") for h in headers])
