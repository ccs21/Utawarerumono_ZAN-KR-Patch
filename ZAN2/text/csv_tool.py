...
class CsvTranslationHelper(QMainWindow)
    ...
    def _build_ui(self) - None
        ...
        self.copy_button = QPushButton(선택 셀 복사, self)
        self.copy_button.clicked.connect(self.copy_selected_cells)
        right_layout.addWidget(self.copy_button)

        self.include_en_checkbox = QCheckBox(복사 시 en 열도 함께 포함, self)
        self.include_en_checkbox.setChecked(False)
        right_layout.addWidget(self.include_en_checkbox)

        help_label = QLabel(
            번역 결과를 아래 칸에 그대로 붙여넣은 뒤 '검증' 또는 'kr 열에 반영'을 누르세요.n
            멀티라인 셀은 반드시 큰따옴표로 묶인 CSV 형태 그대로 붙여넣어야 합니다.,
            self,
        )
    ...
    def _update_ui_state(self) - None
        ...
        self.copy_button.setEnabled(has_file)
    ...
    def load_csv(self, path str) - None
        try
            ...
            try
                ...
                self._show_error(fCSV 읽기 실패n{e})
                return
        except Exception as e
            self._show_error(fCSV 읽기 실패n{e})
            return
        ...
        self.file_label.setText(f파일 {path})
        self.translation_edit.clear()
        self.validation_label.setText(검증 결과 -)
        self._update_selection_label([])
        self._update_ui_state()
        self.status_bar.showMessage(CSV 파일을 불러왔습니다., 5000)
    ...
    def _jump_to_first_blank_kr(self) - None
        ...
        self.next_blank_label.setText(f다음 미번역 행 {visible_row_number}행)

        self.table.scrollToItem(self.table.item(row_idx, self.kr_col or 0), QAbstractItemView.PositionAtCenter)
    ...
    def _update_selection_label(self, row_indices List[int]) - None
        ...
        self.selection_label.setText(f현재 선택 {first}행 ~ {last}행  총 {count}개)
    ...
    def _get_copy_source_column(self) - Optional[int]
        ...
        # Prefer jp if multiple columns are selected
        if self.jp_col is not None and self.jp_col in cols
            return self.jp_col
        return cols[0]

    def _csv_escape_cell(self, value str) - str
        output = io.StringIO()
    ...
    def build_copy_text_from_selection(self, row_indices List[int], source_column int) - str
        if not row_indices
            return 

        lines List[str] = []
        include_en = self.include_en_checkbox.isChecked() and self.en_col is not None

        for row_idx in row_indices
            row = self.rows[row_idx]
            src_value = row[source_column] if source_column  len(row) else 
            lines.append(self._csv_escape_cell(src_value))
            if include_en and self.en_col is not None
                en_value = row[self.en_col] if self.en_col  len(row) else 
                lines.append(self._csv_escape_cell(en_value))
    ...
    def copy_selected_cells(self) - None
        ...
        row_indices = self._get_selected_row_indices()
        if not row_indices
            self._show_error(먼저 복사할 셀행을 선택해주세요.)
            return

        source_column = self._get_copy_source_column()
        if source_column is None
            self._show_error(복사할 열을 결정할 수 없습니다.)
            return

        copied_text = self.build_copy_text_from_selection(row_indices, source_column)
        ...
        first = row_indices[0] + 2
        last = row_indices[-1] + 2
        self.status_bar.showMessage(f{first}행 ~ {last}행을 클립보드에 복사했습니다., 5000)
        QMessageBox.information(
            self,
            복사 완료,
            f선택한 {len(row_indices)}개 행을 클립보드에 복사했습니다.n
            f행 범위 {first}행 ~ {last}행,
        )
    ...
    def _parse_translation_cells(self, text str) - List[str]
        ...
        try
            ...
            raise ValueError(fCSV 형식 파싱 실패 {e}) from e

        cells List[str] = []
        for row in parsed_rows
            # If the pasted content is a normal single-column CSV, each row will have len 1.
            # If the user pasted tabular data by accident, keep all fields so count mismatch is visible.
            for cell in row
                cells.append(cell)
        return cells
    ...
    def validate_translation_input(self) - bool
        ...
        try
            ...
            self.validation_label.setText(f검증 결과 실패 - {e})
        ...
        if actual != expected
            self.validation_label.setText(
                f검증 결과 실패 - 기대 셀 수 {expected}개  입력된 셀 수 {actual}개
            )
            self._show_error(
                f셀 수가 맞지 않습니다.n기대 {expected}개n입력 {actual}개nn
                멀티라인 셀의 큰따옴표가 유지되었는지 확인해주세요.
            )
            return False

        self.validation_label.setText(
            f검증 결과 성공 - 기대 셀 수 {expected}개  입력된 셀 수 {actual}개
        )
        self.status_bar.showMessage(붙여넣은 번역문 검증 성공, 4000)
        return True
    ...
    def apply_translation_to_kr(self) - None
        ...
        if filled_rows
            ...
            proceed = self._confirm(
                f이미 kr 값이 들어있는 행이 있습니다.n행 번호 {sample}nn덮어쓸까요
            )
            if not proceed
                return
        ...
        self.status_bar.showMessage(f{first}행 ~ {last}행의 kr 열에 반영했습니다., 5000)
        self._show_info(f총 {len(target_rows)}개 셀을 kr 열에 반영했습니다.)
    ...
    def _refresh_next_blank_label_only(self) - None
        ...
        if row_idx is None
            ...
            self.next_blank_label.setText(f다음 미번역 행 {row_idx + 2}행)
    ...
    def save_csv(self) - None
        ...
        try
            ...
            self._show_error(fCSV 저장 실패n{e})
            return

        self.is_dirty = False
        self._update_ui_state()
        self.status_bar.showMessage(f저장 완료 {self.file_path}, 5000)
        self._show_info(CSV 저장이 완료되었습니다.n백업 파일(.bak)도 함께 생성했습니다.)
