...
class CsvTranslationHelper(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        help_label = QLabel(
            "번역 결과를 아래 칸에 그대로 붙여넣은 뒤 '검증' 또는 'kr 열에 반영'을 누르세요.\n"
    ...
    def copy_selected_cells(self) -> None:
        ...
        QMessageBox.information(
            self,
            "복사 완료",
            f"선택한 {len(row_indices)}개 행을 클립보드에 복사했습니다.\n"
            f"행 범위: {first}행 ~ {last}행",
        )
    ...
    def apply_translation_to_kr(self) -> None:
        ...
        self._show_info(f"총 {len(target_rows)}개 셀을 kr 열에 반영했습니다.")
    ...
    def save_csv(self) -> None:
        ...
        self.is_dirty = False
        self._update_ui_state()
        self.status_bar.showMessage(f"저장 완료: {self.file_path}", 5000)
        self._show_info("CSV 저장이 완료되었습니다.\n백업 파일(.bak)도 함께 생성했습니다.")
