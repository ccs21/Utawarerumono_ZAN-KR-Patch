...
class CsvTranslationHelper(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.remaining_blank_label = QLabel("남은 미번역 행: -", self)
        right_layout.addWidget(self.remaining_blank_label)
    ...
    def _count_blank_kr_rows(self) -> int:
        if self.kr_col is None:
            return 0
        count = 0
    ...
    def _jump_to_first_blank_kr(self) -> None:
        remaining = self._count_blank_kr_rows()
        self.remaining_blank_label.setText(f"남은 미번역 행: {remaining}행")
